...
public class AnnotUnaryPayloadElem<K extends PayloadTypeKind> extends UnaryPayloadElem<K> implements AnnotPayloadElem<K>
{
	...
	@Override
	public AnnotUnaryPayloadElem<K> clone(AstFactory af)
	{
		PayloadVarNode payvar = isAnnotated() ? this.payvar.clone(af) : null;  // Discards del (cf. ScribNode.clone)
		PayloadElemNameNode<K> name = ScribUtil.checkNodeClassEquality(this.name, this.name.clone(af));  // Returns second arg
		return ((AnnotAstFactory) af).AnnotUnaryPayloadElem(this.source, payvar, name);
	}
